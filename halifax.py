...
url = "https://assets.lloydsbankbusiness.com/affordabilitycalculator/"
...
headers ={
	...
	"Referer":"https://assets.lloydsbankbusiness.com/affordabilitycalculator/",
...
def get_mortgage_data(data, num_of_dependants):
	...
	Help_To_Buy = "Yes" if Mortgage_Type == "Shared Ownership" or Mortgage_Type =="Shared Equity / Help To Buy" else "No"
	...
	mortgage_data["__VIEWSTATEGENERATOR"] = "776369CD"
	...
	if Loan_Purpose == "Purchase":
		mortgage_data["purpose"] = "P"
	else:
		mortgage_data["purpose"] = "RCR"

	mortgage_data["ddlhelptobuy"] = Help_To_Buy
...
def main(config_file):
    ...
    if response.status_code==200:
    	try:
    		soup = BeautifulSoup(response.content, 'html.parser')
    		loan_amount = soup.select_one("#loanamount").text
    		max_borrowing = soup.select_one("#maxloanamount").text
    		print("Result : PASS")
