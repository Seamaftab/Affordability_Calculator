...
url = 'https://buckinghamshirebs.asanto.com/engine/mortgage/buckinghamshire_hosted_resi_live/'

def get_mortgage_data(data):
    mortgage_data = {}
    
    valid_regions = ["East Midlands", "East of England", "Greater London", "North East", "North West", "South East", "South West", "West Midlands", "Yorkshire and Humberside", "Wales"]
    ...
    if repayment_type == "Repayment":
        ...
        mortgage_data["repayment_type"] =  "Interest only part and part"
...
def get_applicants(data):
    ...
    for applicant in applicant_details:
        ...
        EmpDetails = applicant.get("Employment Details", [])
        AdditionalIncome = applicant.get("Additional Income (Annual)", [])
        Outgoings = applicant.get("Outgoings", [])
        Household = Outgoings.get("Household", [])
        ...
        Buckingham = {
            ...
            "employment_status": EmpDetails.get("Employment Status"),
            "dependants_0_to_5": 0,
            "dependants_6_to_11": 0,
            "dependants_12_to_17": 0,
            "dependants_18_and_over": num_of_dependants,
            "annual_gross_salary": str(EmpDetails.get("Basic Annual Income")),
            "shareholding_percentage": 0,
            "annual_net_profit_current": EmpDetails.get("Last Year's Net Profits", 0),
            "annual_net_profit_previous": EmpDetails.get("Previous Year's Net Profits", 0),
            "annual_net_profit_period_3": EmpDetails.get("3rd Year's Net Profits", 0),
            "annual_directors_salary_current": EmpDetails.get("Last Year's Salary", 0),
            "annual_directors_salary_previous": EmpDetails.get("Previous Year's Salary", 0),
            "annual_directors_salary_period_3": EmpDetails.get("3rd Year's Salary", 0),
            "annual_gross_dividend_current": EmpDetails.get("Last Year's Dividends", 0),
            "annual_gross_dividend_previous": EmpDetails.get("Previous Year's Dividends", 0),
            "annual_gross_dividend_period_3": EmpDetails.get("3rd Year's Dividends", 0),
            "annual_net_profit_ltd_latest_period": EmpDetails.get("Last Year's Retained Profits", 0),
            "annual_net_profit_ltd_period_2": EmpDetails.get("Previous Year's Retained Profits", 0),
            "annual_net_profit_ltd_period_3": EmpDetails.get("3rd Year's Retained Profits", 0),
            ...
            "annual_bonus": AdditionalIncome.get("Regular Bonus", 0),
            "bonus_payment_frequency": "",
            "annual_commission": 0,
            "commission_payment_frequency": "",
            "annual_overtime": AdditionalIncome.get("Irregular Overtime", 0),
            ...
            "total_credit_card_balance": str(0 if Outgoings.get("Credit Commitments") in ["null", None] else Outgoings.get("Credit Commitments")),
            "credit_card_balance_to_be_repaid": 0,
            "annual_loan_payments": 0,
            "annual_household_expenses": Household.get("Mortgage / Rent", 0),
            "annual_gas_electric": Household.get("Electricity", 0),
            "annual_water": Household.get("Electricity", 0) if Household.get("Electricity") else 0,
            "annual_tv_internet": Household.get("Television", 0),
            "annual_mobile_landlines": Outgoings["Living Costs"].get("Mobile Phone", 0),
            "annual_council_tax": 0,
            "annual_home_insurances": 0,
            "annual_life_insurances": 0,
            "annual_ground_rent_service_charge": 0,
            "annual_travel_transport": 0 if Outgoings.get("Transport") in ["null", None] else Outgoings.get("Transport"),
            "annual_maintenance_payments": 0,
            "annual_childcare": 0 if Outgoings.get("Children") in ["null", None] else Outgoings.get("Children"),
            "annual_car_upkeep": 0,
        ...
        applicant_data.append(Buckingham)
